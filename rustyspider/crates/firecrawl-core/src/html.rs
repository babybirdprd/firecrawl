...
pub async fn extract_base_href(html: String, url: String) -> anyhow::Result<String> {
  let res = task::spawn_blocking(move || _extract_base_href(&html, &url))
    .await
    .map_err(|e| {
      anyhow!(format!("extract_base_href join error: {e}"),
      )
    })?;
...
pub async fn extract_links(html: Option<String>) -> anyhow::Result<Vec<String>> {
  task::spawn_blocking(move || {
    ...
    let anchors: Vec<_> = document
      .select("a[href]")
      .map_err(|_| anyhow!("Failed to select links"))?
      .collect();
  ...
  .await
  .map_err(|e| {
    anyhow!(format!("extract_links join error: {e}"),
    )
  })?
}
...
pub async fn extract_metadata(html: Option<String>) -> anyhow::Result<HashMap<String, Value>> {
  task::spawn_blocking(move || {
    let html = match html {
      Some(h) => h,
    ...
    _extract_metadata(&html).map_err(|e| anyhow!(e))
  })
  .await
  .map_err(|e| {
    anyhow!(format!("extract_metadata join error: {e}"),
    )
  })?
}
...
pub async fn transform_html(opts: TransformHtmlOptions) -> anyhow::Result<String> {
  let res = task::spawn_blocking(move || _transform_html_inner(opts))
    .await
    .map_err(|e| {
      anyhow!(format!("transform_html join error: {e}"),
      )
    })?;
...
pub async fn get_inner_json(html: String) -> anyhow::Result<String> {
  let res = task::spawn_blocking(move || _get_inner_json(&html))
    .await
    .map_err(|e| {
      anyhow!(format!("get_inner_json join error: {e}"),
      )
    })?;

  res.map_err(|_| anyhow!("Failed to get inner JSON"))
}
...
pub async fn extract_attributes(
  html: String,
  options: ExtractAttributesOptions,
) -> anyhow::Result<Vec<ExtractedAttributeResult>> {
  let res = task::spawn_blocking(move || _extract_attributes(&html, &options))
    .await
    .map_err(|e| {
      anyhow!(format!("extract_attributes join error: {e}"),
      )
    })?;
...
pub async fn extract_images(html: String, base_url: String) -> anyhow::Result<Vec<String>> {
  let res = task::spawn_blocking(move || _extract_images(&html, &base_url))
    .await
    .map_err(|e| {
      anyhow!(format!("extract_images join error: {e}"),
      )
    })?;
...
pub async fn post_process_markdown(markdown: String) -> anyhow::Result<String> {
  ...
  .await
  .map_err(|e| {
    anyhow!(format!("post_process_markdown join error: {e}"),
    )
  })?;
